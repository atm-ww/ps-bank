-- client/main.lua
local npcData = {
    { position = vector3(146.06, -1042.62, 28.37), heading = -20.0 },  
    { position = vector3(310.31, -280.83, 53.16), heading = -20.0 },  
    { position = vector3(-354.94, -51.94, 48.04), heading = -20.0 },  
    { position = vector3(-1213.96, -334.92, 36.78), heading = 20.0 },  
    { position = vector3(-2959.32, 479.66, 14.70), heading = 90.0 },  
    { position = vector3(1178.15, 2709.80, 37.09), heading = 170.0 },  
    { position = vector3(242.96, 224.35, 105.29), heading = 170.0 },  
    { position = vector3(-112.26, 6471.21, 30.63), heading = 140.0 },  
}

function createBankerNPC(position, heading)
    local model = "a_m_y_business_02" -- NPC 模型
    RequestModel(model)
    while not HasModelLoaded(model) do
        Wait(500) -- 等待模型加載
    end

    local npc = CreatePed(5, model, position.x, position.y, position.z, heading, false, true)
        ...
        options = {
            {
                ...
                label = "訪問銀行",
                action = function()
                    TriggerEvent('ps-banking:client:open:bank')
                end,
            },
        },
        distance = 2.5,
...
Citizen.CreateThread(function()
    for _, npc in ipairs(npcData) do
        createBankerNPC(npc.position, npc.heading)
    end
end)
